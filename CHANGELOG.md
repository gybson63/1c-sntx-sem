...
- Поиск по пустому домену (например `domain=bsp` без Ingest BSP): больше не падает с `division by zero`, возвращает пустой результат.
- Ранжирование `domain=all`: per-domain pool масштабируется по размеру домена (не обрезается до 5 чанков для больших `platform_api`); title bonus учитывает русскую морфологию и префиксы (`строку` ↔ `стр`).
- Rebuild Index на Windows: `OSError [WinError 1920]` при битой junction `help_chunks` — удаление таблицы через `help_chunks.lance` и безопасная очистка legacy-путей.
- OOM при rebuild ~28k чанков: векторы не дублируются в `meta_rows`/`chunks_meta.json`; потоковая запис метаданных; опциональный пропуск IVF `create_index`.
- `/status` и `/search`: повреждённый LanceDB (неполный rebuild) определяется до поиска, с понятным сообщением вместо сырой ошибки Lance.
- Сообщения об OOM на этапе IVF LanceDB (после 100% эмбеддингов) с подсказкой `build_vector_index: false`.
- OOM при поиске `domain=all` в Docker без IVF: один Lance dense-запрос вместо N+1; per-domain покрытие через BM25.
- HTTP `/search`: понятные сообщения об OOM при поиске (RAM Docker, выбор домена, `dense_top_k`/`bm25_top_k`) вместо сырого Lance stack.
...
- Построение индекса: запись в LanceDB пакетами вместо одного bulk-вызова — меньше пик RAM при ~28k чанках и e5-base.
- Web-UI `/admin`: при «Failed to fetch» во время индексации — повтор опроса задачи и понятное сообщение вместо мгновенной остановки.
...
- Поиск по всем доменам (`domain=all`): глобальный dense + per-domain BM25 (без повторных Lance-запросов на домен) — меньше RAM в Docker; релевантные статьи БСП не теряются из-за объёма `platform_api`.
...
- Индексация разделена на два канала: `semantic_text` (описание/параметры/возврат/синтаксис) для эмбеддингов и `lexical_text` (имена/пути) для BM25 и title bonus. Разговорные запросы вроде «строку в массив» опираются на смысл описания, а не только на совпадение имени. **Требуется пересборка индекса**.
